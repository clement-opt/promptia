...
export function buildStructuredPrompt({
  taskType, customTaskType, objective,
  domain, customDomain, subdomain, customSubdomain,
  size, customSize, audience, customAudience, level, customLevel, situation,
  documentType, customDocumentType, length, customLength,
  tone, customTone, structure, customStructure,
  options,
}) {
  // Résoudre les valeurs "Autre" -> valeur custom
  const r = (v, c) => (v === 'Autre' ? (c || '') : v) || ''
  taskType     = r(taskType, customTaskType)
  domain       = r(domain, customDomain)
  subdomain    = r(subdomain, customSubdomain)
  size         = r(size, customSize)
  audience     = r(audience, customAudience)
  level        = r(level, customLevel)
  documentType = r(documentType, customDocumentType)
  length       = r(length, customLength)
  tone         = r(tone, customTone)
  structure    = r(structure, customStructure)
...
export function buildExpertPrompt({
  ...
  // Résoudre les valeurs "Autre" -> valeur custom
  const r = (v, c) => (v === 'Autre' ? (c || '') : v) || ''
  const roleLabel = r(role, customRole)
  personality  = r(personality, customPersonality)
  taskType     = r(taskType, customTaskType)
  domain       = r(domain, customDomain)
  subdomain    = r(subdomain, customSubdomain)
  size         = r(size, customSize)
  ...
  if (askQuestions) {
    lines.push('## CONSIGNE PRÉALABLE')
    lines.push("Avant de répondre, pose-moi 3 à 5 questions pour mieux comprendre mon besoin et affiner ta réponse.")
    lines.push('')
  }
